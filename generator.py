...
def make_code(r, b):
    ...
    m = 6 * r
    ncells = ell * m
    # l qubits are 0 to ncells-1
    # r qubits are ncells to 2*ncells-1
    def cell(i, j):
        return (i % ell) * m + (j % m)
...
def reduce_vec(vec, rref, pivot_cols):
    ...
    for i, pc in enumerate(pivot_cols):
        if vec[pc]:
            vec = (vec + rref[i]) % 2
    return vec
# finds k logical ops that commute with the other check type
# but aren't stabilizers
...
def save_code(hx, hz, r, b):
    # dumps matrices for gurobi load
    os.makedirs("codes", exist_ok=True)
    np.save(
        "codes/hx_r" + str(r) + "_b" + str(b) + ".npy",
        hx
    )
    np.save(
        "codes/hz_r" + str(r) + "_b" + str(b) + ".npy",
        hz
    )
if __name__ == "__main__":
    ...
    for r, b in test_cases:
        ...
        if check.any():
            ...
            continue
        # Every check should touch 6 qubits
        rw = hx.sum(axis=1)
        rwz = hz.sum(axis=1)
        ...
        print(
            "Hx Hz^T =",
            "ZERO" if not check.any() else "NONZERO"
        )
        print(
            "Hx row weights:",
            np.unique(rw, return_counts=True)
        )
        print(
            "Hz row weights:",
            np.unique(rwz, return_counts=True)
        )
        print(
            "r =", r,
            "b =", b,
